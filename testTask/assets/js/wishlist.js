document.addEventListener("DOMContentLoaded", () => {
  ...
  function createWishlistItem(product) {
    let wislistItem = document.createElement("div");
    wislistItem.classList.add("wishlist-item");
    ...
    function removeItem(productId) {
      ...
      if (productIndex !== -1) {
        ...
        toast("Item removecd from wishlist")
        setTimeout(()=>{
            location.reload()
        },500)
    ...
    image.append(img);
    wislistItem.append(image, title, category, price, removeBtn);
    let wishlistContainer = document.querySelector(".wishlist");
    wishlistContainer.appendChild(wislistItem);
